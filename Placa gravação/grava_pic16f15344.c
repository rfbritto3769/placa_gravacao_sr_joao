...
void grava_pic16f15344(void)
{
    ...
    unsigned char i=0,a,b,c,d,AddrExtend=0;
		...
		do
		{
			...
			while(IndexBuffer<512&&!ret&&!AbortaGr)
			{
				...
				if(MemoryBuffer[IndexBuffer]==':')
				{
					...
					else if(AddrExtend && RecordType==0x00&&Address>=0xe000&&Address<=0xe1ff) 		ProgData344_1();
